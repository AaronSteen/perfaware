...
Debug_PrintUpdatedRegisterState(struct decoded_inst *DecodedInst, union registers *NewRegisters,
                                   union registers *OldRegisters, int Clocks)
{
    ...
    // Clocks
    AppendToRegisterStateBuf(&Cursor, OnePastLast, "Clocks: +%d = %d | ", 
                             DecodedInst->Clocks, Clocks);
...
Debug_PrintFinalRegisterState(union registers *Registers)
{
    ...
    for(int i = 0; i < NUMBER_OF_WORD_SIZED_REGISTERS - 1; i++)
    {
        if(Registers->WordRegisters[i])
        {
            ...
            sprintf_s(RegisterStateBuf, sizeof(RegisterStateBuf), "%8s: ", RegName);
            OutputDebugStringA(RegisterStateBuf);
            sprintf_s(RegisterStateBuf, sizeof(RegisterStateBuf), "0x%04x (%u)", Value, Value);            
            OutputDebugStringA(RegisterStateBuf);
            OutputDebugStringA("\n\n");
        }
    }
...
struct inst_stream
Win32_LoadInstStream(HANDLE HeapHandle, HANDLE FileHandle, u8 *Memory)
{
...
void
Win32_InitializeMemory(u8 **Memory, struct inst_stream *InstStream, HANDLE FileHandle)
{
    ...
    if(HeapHandle)
    {
        *Memory = (u8 *)HeapAlloc(HeapHandle, HEAP_ZERO_MEMORY, (1024 * 1024));
        if(Memory)
        {
            *InstStream = Win32_LoadInstStream(HeapHandle, FileHandle, *Memory);
        }
        else
        {
            Debug_OutputErrorMessage("HeapAlloc failed");
            exit(1);
        }
...
Group2Decode(struct decoded_inst *DecodedInst)
{
    ...
    DecodedInst->OperandTwo = ReadImmField(DecodedInst, ImmBits, ImmField, CareAboutSignExtend, IsSigned);
    if(IsDirectAddressInstruction(DecodedInst))
    {
...
GetEffectiveAddress(struct decoded_inst *DecodedInst, union registers *Registers)
{
    ...
    u16 ToReturn = 0;
    if(DecodedInst->Displacement) 
    { 
        DecodedInst->Clocks += 4; 
    }
    switch(DecodedInst->RorM)
    {
        ...
        case 6:
            {
                ToReturn = Registers->BP + DecodedInst->Displacement;
                DecodedInst->Clocks += 5;
                return(ToReturn);
            } break;
...
DoInstruction(struct decoded_inst *DecodedInst, union registers *Registers, u8 *Memory)
{
    switch(DecodedInst->DecodeGroup)
    {
        ...
        case 5:
        {
            // [.... .reg] (no trailing immediate value)
        } break;

        case 6:
        { 
            // [.... w reg] [data] [data] (up to two bytes for trailing imm value)
            DecodedInst->Clocks += 4;
            if(DecodedInst->IsWord)
            {
...
void
Win32_WriteImage(LPCSTR Filename, SIZE_T FileSize, u8 *Pixels)
{
    HANDLE ImageHandle = CreateFileA(Filename, 
    ...
    {
        // Todo: Error handling
        bool WriteResult = WriteFile(ImageHandle, Pixels, 
                                       FileSize, 
                                       0, 0);
        if(!WriteResult)
        {
            Debug_OutputErrorMessage("Image write failed");
            exit(1);
        }
    }
    else
    {
        Debug_OutputErrorMessage("Create image file failed");
        exit(1);
    }
}
